...
ChatPollServerClass.prototype.uploadFile = function(file, fileType, parentId, rank, _ticketReplyWindowId, sendToChat, _toKb, _toTicketComment, _sendToChatWindowId) {
    ...
    if (true || !thisClass.pollIsActive)
    {
        try
        {
            ...
            var myUserName = lz_global_base64_url_encode(this.chosenProfile.login_name);
            var myToken = lz_global_base64_url_encode(sha256(DataEngine.token));
            var myRequestType = lz_global_base64_url_encode('intern');
            var myAction = lz_global_base64_url_encode('send_file');
            var myLoginId = lz_global_base64_url_encode(thisClass.loginId);
            var myFileType = lz_global_base64_url_encode(fileType);
            */
