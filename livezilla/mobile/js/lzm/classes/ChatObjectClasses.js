...
LzmCustomInputs.prototype.getInputHtmlRow = function(inp) {
    ...
    return '<tr id="inp_row_'+inp.id+'">' +
        ...
        '<td id="ivalf_' + inp.id +'" style="text-align:center;">'+lzm_inputControls.createButton('ival_' + inp.id, '','ServerConfiguration.SetValidation(\''+inp.id+'\');', tid('validation'), '<i class="fa fa-cog '+icon_col+'" id="ivali_'+inp.id+'"></i>', '', {padding:'11px 20px 10px 20px'}, '', '','d')+'</td>' +
        '</tr>';
};
...
Chat.prototype.GetVisibleOperatorCount = function(){
    ...
    for(var i = 0;i<this.Members.length;i++)
    {
        ...
        var isInv = (typeof this.Members[i].s != 'undefined' && this.Members[i].s=='2');
