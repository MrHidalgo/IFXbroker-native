...
function getQrdDownloadUrl(resource) {
    ...
    downloadUrl += 'a=' + lzm_commonTools.pad(Math.floor(Math.random() * 1048575).toString(16), 5) +
        '&file=' + encodeURI(resource.ti) + '&id=' + encodeURI(resource.rid);
    return downloadUrl;
}
...
function sendQrdPreview(_resourceId, _chatPartner) {
    ...
    if (resource != null)
    {
        ...
        if (false && (IFManager.IsAppFrame || IFManager.IsMobileOS) && !IFManager.IsDesktopApp())
        {
            /*
            chatText = (chatText != '') ? chatText + ' ' : chatText;
            if ($.inArray(resource.ty, ['2', '3', '4']) != -1)
            {
                SendChat(resourceHtmlText, _chatPartner, '');
            }
            else
            {
                ...
                ChatManager.SaveEditorInput(_chatPartner, chatText + resourceTextText);
            }
            */
        }
        else
...
function showTranslationStringsLoadingDiv() {
    ...
    $('#translation-strings-loading').css({position: 'absolute',left:0,top:0,bottom:'2px',right:0,'background-color': '#fff', 'background-position': 'center', 'z-index': 1000});
}
...
function initLinkGenerator(e){
    ...
    if (DataEngine.operators.getOperator(lzm_chatDisplay.myId).level == 1) {
        $.getScript('js/lzm/classes/LinkGeneratorClass.js', function( data, textStatus, jqxhr ) {
            ...
            $.getScript('../templates/jscript_ahjxcd3a/icons.js',function(){});
