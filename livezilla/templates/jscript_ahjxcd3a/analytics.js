...
function TableGraph(_id, _config, _rc) {
        ...
        container.selectAll('.table-pie-wrapper')
            ...
            .each(function (d) {
                if (d.pie) {
                    ...
                    if ((others > 0) || (d.totalRef - sum) > (d.totalRef - d.totalRef/1000)){
                        others += (d.totalRef - sum);
                        pieData.push([tid['others'], others]);
                    }
                    d3.select(this).append('div').attr('id', id + d.key + 'pieChart').classed('pie-wrapper', true)
                        .append('div').attr('id', id + d.key + 'c3wrapper').classed('c3-pie-wrapper', true);
                    Pie.bake('#' + id + d.key + "c3wrapper", pieData);
                }
            })
    }
    ...
    function drawTable(td, ti, ta) {
        ...
        d3.select(ta[ti]).select('.table-wrapper').selectAll('#table-headline-' + td.key)
            .data([td])
            .enter()
            .append('span')
            .classed('table-headline', true)
            .attr('id', 'table-headline-' + td.key)
            .html(function (d) {
                if (d.tid)
                    return tid[d.tid];
                else
        ...
        var table = d3.select(ta[ti]).select('.table-wrapper').selectAll('#table-' + td.key)
            .data([td])
            .enter()
            .append('table')
            .classed('tops-table', true)
            .attr('id', 'table-' + td.key);
        ...
        var showMore = d3.select(ta[ti]).select('.table-wrapper').selectAll('#table-' + td.key + "-show-more")
            .data(["show-more"]);
        showMore.enter()
            .append('div')
            .attr('id', '#table-' + td.key + "-show-more")
    ...
    function getData(_callback, _postData, _range, _method, _url){
        ...
        $.ajax({
            ...
            success: function(_resultData, status, xhr){
                ...
                // if(_postData.request === 'ticketsBar')
