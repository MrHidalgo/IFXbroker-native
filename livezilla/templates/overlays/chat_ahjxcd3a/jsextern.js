...
function lz_chat_set_connecting(_connecting,_id,_waitingHTML,_waitingDelay){
    ...
    document.getElementById("lz_chat_content_box").style.bottom = (OverlayChatWidgetV2.__Connecting) ? "120px" : "80px";
...
function lz_chat_decline_request(_id,_operator,_stateChange,_result,_closeWindow){
    ...
    if(node != null && node.style.display != 'none')
    {
        if(!_operator)
        {
            ...
            lz_tracking_action_result('chat_request',_result,false,OverlayChatWidgetV2.AddParameters());
        }
    ...
    if(lz_overlay_wm == null && !OverlayChatWidgetV2.__ModeCombi)
    {
        ...
        for(var y=lz_ovlel.length-1;y>=0;y--)
        {
            ...
            if(lz_ovlel[i].type != 'wm')
            {
                ...
                {
                    clnd.style.right = (OverlayChatWidgetV2.__SizeWidth*0.5)-lz_border_radius + 'px';
                    clnd.style.boxShadow = 'none';
                }
...
function lz_chat_poll_parameters(){
    // comp 8.x
    return OverlayChatWidgetV2.AddParameters();
}
...
OverlayChatWidgetV2.GetWMWidth = function(_service){
    ...
    var base = ((OverlayChatWidgetV2.__ModeTextInline) ? 77 : 20) * OverlayChatWidgetV2.__Ratio;
    var letterwidth = 9*OverlayChatWidgetV2.__Ratio;
    var bl = base + (maxlength*letterwidth);
    return Math.max(Math.min(bl,maxwidth),100);
};
...
OverlayChatWidgetV2.FlipWM = function(){
    ...
    setTimeout('document.getElementById(\'livezilla_wm\').style.transform=\'\';lz_remove_class(document.getElementById(\'livezilla_wm\'),\'lz_anim_flip\');',500);
};
...
OverlayChatWidgetV2.UpdateUI = function(){
    ...
    document.getElementById("lz_overlay_module_custom").style.visibility = (lz_session.OVLWMSElem.indexOf('custom') === 0) ? "visible" : "hidden";
...
OverlayChatWidgetV2.CaptureScreen = function(_delayed){

    document.getElementById('lz_overlay_chat').style.display='none';

    if(!_delayed)
    {
        setTimeout('OverlayChatWidgetV2.CaptureScreen(true);',200);
        return
    }
    ...
    reader.onloadend = function() {
        ...
        var params = 'form_userfile='+lz_global_base64_url_encode(OverlayChatWidgetV2.Screenshot);
        ...
        lz_post(lz_poll_server+'upload.php?f=MQ__&ptl=' + LiveZillaData.Language,params,function(){lz_load_inputs = true;lz_tracking_poll_server(1021);});
        ...
        var oi = setInterval(function(){
            ...
            if(opacity <= 0)
            {
                ...
                clearInterval(oi);
                document.getElementById('lz_overlay_chat').style.display='block';
            }
        },30);
    };
};
...
OverlayChatWidgetV2.SetCustomURL = function(){
    ...
    if(document.getElementById("lz_overlay_module_custom").getElementsByTagName("iframe")[0].src != url)
        document.getElementById("lz_overlay_module_custom").getElementsByTagName("iframe")[0].src = url;
